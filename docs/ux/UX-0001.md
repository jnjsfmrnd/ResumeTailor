...
### Feature Summary
This UX design covers the single-user resume tailoring workflow for PDF upload, GitHub Models credential selection, optional masked user key entry, optional cover letter generation, editable review, and export. V1 uses one hosted QA validation target, while the UX remains focused on one user completing the workflow at a time.
...
### Success Criteria
- User can complete setup and reach an editable draft from a single primary screen without a multi-page wizard.
...
### Screen 1: Upload and Settings (user key mode shown)
```
+-------------------------------------------------------------+
| ResumeTailor                                                |
|-------------------------------------------------------------|
...
| [ GPT-5.1 v ]                                               |
|                                                             |
| GitHub Models API Key (shown only in user key mode)         |
| [ *********************** ]                                 |
...
### Screen 2: Review and Edit (`Resume and cover letter` state)
```
+-------------------------------------------------------------+
...
In `Resume only` mode, the cover-letter row and the `Download cover letter PDF` action are absent.
